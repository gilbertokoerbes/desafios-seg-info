...
func find_hash(i_num int) {
	...
	concatenatedString := fmt.Sprintf("%s%d", Name, i_num)
	...
	if leadingZeroBits > 29 {
		fmt.Printf("concatenatedString > 29: %d\n", concatenatedString)
...
func mainLoop() {
	for i := 0; i < i+1; i++ {
		go find_hash(i) // Call the main function
	}
}
...
func main() {
	...
	// 	fmt.Printf("os.Getenv('GOMAXPROCS') %d", os.Getenv("GOMAXPROCS"))
	// }

	mainLoop() // Start the main loop in a separate goroutine
